...
(
    ...
    localparam UNROLLED_STAGES  = (NOF_BUTTERFLY_UNITS==2) ? 1 :
                                  (NOF_BUTTERFLY_UNITS==8) ? 2 :
                                  (NOF_BUTTERFLY_UNITS==32) ? 3 : 4;
    ...
    logic [DATA_WIDTH-1:0] unrolled_stage_din_0 [UNROLLED_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] unrolled_stage_din_1 [UNROLLED_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] unrolled_stage_din_2 [UNROLLED_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] unrolled_stage_din_3 [UNROLLED_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [UNROLLED_STAGES-1:0] unrolled_stage_data_valid_in;

    logic [DATA_WIDTH-1:0] unrolled_stage_dout_0 [UNROLLED_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] unrolled_stage_dout_1 [UNROLLED_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] unrolled_stage_dout_2 [UNROLLED_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] unrolled_stage_dout_3 [UNROLLED_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [UNROLLED_STAGES-1:0] unrolled_stage_data_valid_out; 

    // Assign inputs to internal signals
    generate;
    if (UNROLLED_STAGES > 1) begin
        ...
        assign unrolled_stage_din_0[UNROLLED_STAGES-1] = intt_i ? mdc_dout_0 : unrolled_stage_dout_0[UNROLLED_STAGES-2];
        assign unrolled_stage_din_1[UNROLLED_STAGES-1] = intt_i ? mdc_dout_1 : unrolled_stage_dout_1[UNROLLED_STAGES-2];
        assign unrolled_stage_din_2[UNROLLED_STAGES-1] = intt_i ? mdc_dout_2 : unrolled_stage_dout_2[UNROLLED_STAGES-2];
        assign unrolled_stage_din_3[UNROLLED_STAGES-1] = intt_i ? mdc_dout_3 : unrolled_stage_dout_3[UNROLLED_STAGES-2];
        assign unrolled_stage_data_valid_in[UNROLLED_STAGES-1] = intt_i ? data_mdc_dout_valid[0] : unrolled_stage_data_valid_out[UNROLLED_STAGES-2];
    ...
    assign mdc_din_0 = intt_i ? data0_i : unrolled_stage_dout_0[UNROLLED_STAGES-1];
    assign mdc_din_1 = intt_i ? data1_i : unrolled_stage_dout_1[UNROLLED_STAGES-1];
    assign mdc_din_2 = intt_i ? data2_i : unrolled_stage_dout_2[UNROLLED_STAGES-1];
    assign mdc_din_3 = intt_i ? data3_i : unrolled_stage_dout_3[UNROLLED_STAGES-1];
    assign data_mdc_din_valid = intt_i ? data_valid_i : unrolled_stage_data_valid_out[UNROLLED_STAGES-1];
    ...
    logic [DATA_WIDTH-1:0] twiddle1_rom2mdc[UNROLLED_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] twiddle2_rom2mdc[UNROLLED_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] twiddle3_rom2mdc[UNROLLED_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];

    for (genvar i=0; i<UNROLLED_STAGES; ++i) begin : g_pre_mdc

        if ((NOF_BUTTERFLY_UNITS >> (i+1)*2) > 0) begin
        ...
        // Read twiddle factors for stage 
        localparam memfile1_ntt = $sformatf("%s/twiddle_stage_rom1_%0d.mem",TWIDDLE_MEM_PATH,i);
        localparam memfile2_ntt = $sformatf("%s/twiddle_stage_rom2_%0d.mem",TWIDDLE_MEM_PATH,i);
        localparam memfile3_ntt = $sformatf("%s/twiddle_stage_rom3_%0d.mem",TWIDDLE_MEM_PATH,i);
        localparam memfile1_intt = $sformatf("%s/twiddle_inv_stage_rom1_%0d.mem",TWIDDLE_MEM_PATH,i);
        ...
        for (genvar j=0; j<NOF_BUTTERFLY_UNITS; ++j) begin
            ...
            logic [NOF_BUTTERFLY_UNITS/2-1:0] pre_mdc_stage_2bfus_data_valid_out;
            assign unrolled_stage_data_valid_out[i] = pre_mdc_stage_2bfus_data_valid_out[0];

            for (genvar j=0; j<(NOF_BUTTERFLY_UNITS/2); ++j) begin
                ntt_r4mdc_uni_stage_perm #(
                    .DATA_WIDTH(DATA_WIDTH),
                    .N(N/(NOF_BUTTERFLY_UNITS/2)),
                    .QINT(QINT),
                    .QDASH(QDASH),
                    .FIFO_DEPTH(N/(4**(2+i))),
                    .REDUCTION(REDUCTION), // Choose between "BARRETT", "MONTGOMERY", "SOLINA32" and "SOLINA64" ToDo
                    ...
                    .TWIDDLE2_OPT(TWIDDLE2_OPT),
                    .STAGE(i)             // Choose between [0,LOG2(N)-1]
                ) U_R4_MDC_STAGE_2BFUS (
                    .clk_i(clk_i),
                    .rst_i(rst_i),
                    .intt_i(intt_i),
                    .data0_i(unrolled_stage_din_0[i][2*j+1:2*j]),
                    .data1_i(unrolled_stage_din_1[i][2*j+1:2*j]),
                    .data2_i(unrolled_stage_din_2[i][2*j+1:2*j]),
                    .data3_i(unrolled_stage_din_3[i][2*j+1:2*j]),
                    .data_valid_i(unrolled_stage_data_valid_in[i]),

                    .data0_o(unrolled_stage_dout_0[i][2*j+1:2*j]),
                    .data1_o(unrolled_stage_dout_1[i][2*j+1:2*j]),
                    .data2_o(unrolled_stage_dout_2[i][2*j+1:2*j]),
                    .data3_o(unrolled_stage_dout_3[i][2*j+1:2*j]),
                    .data_valid_o(pre_mdc_stage_2bfus_data_valid_out[j]),

                    .twiddle_raddr_o(twiddle_raddr),
                    .twiddle0_i(twiddle0[1:0]),
                    .twiddle1_i(twiddle1_rom2mdc[i][2*j+1:2*j]),
                    .twiddle2_i(twiddle2_rom2mdc[i][2*j+1:2*j]),
                    .twiddle3_i(twiddle3_rom2mdc[i][2*j+1:2*j])
                );
            end
        end
    end
  
    for (genvar i=1; i<UNROLLED_STAGES; ++i) begin
      if ( i != UNROLLED_STAGES-1) begin
    ...
    localparam int unsigned N_MDC = N/(2*NOF_BUTTERFLY_UNITS);
    logic [DATA_WIDTH-1:0] twiddle1_mux2mdc[NOF_BUTTERFLY_UNITS-1:0][$clog2(N_MDC)/2-1:0];
    logic [DATA_WIDTH-1:0] twiddle2_mux2mdc[NOF_BUTTERFLY_UNITS-1:0][$clog2(N_MDC)/2-1:0];
    logic [DATA_WIDTH-1:0] twiddle3_mux2mdc[NOF_BUTTERFLY_UNITS-1:0][$clog2(N_MDC)/2-1:0];
    logic [DATA_WIDTH-1:0] twiddle1_rom2mux[NOF_BUTTERFLY_UNITS-1:0][$clog2(N_MDC)/2-1:0];
    logic [DATA_WIDTH-1:0] twiddle2_rom2mux[NOF_BUTTERFLY_UNITS-1:0][$clog2(N_MDC)/2-1:0];
    logic [DATA_WIDTH-1:0] twiddle3_rom2mux[NOF_BUTTERFLY_UNITS-1:0][$clog2(N_MDC)/2-1:0];
    
    logic [$clog2(2*4**($clog2(N_MDC)/2))-1:0] twiddle_stage_rom_raddr [NOF_BUTTERFLY_UNITS-1:0][$clog2(N_MDC)/2-1:0];
    for (genvar i=0; i<(NOF_BUTTERFLY_UNITS); ++i) begin : g_mdc_par
        ...
        for (genvar j=0; j < $clog2(N/(2*NOF_BUTTERFLY_UNITS))/2; j++) begin
            ...
            logic [$clog2(2*(4**j))-1:0] twiddle_stage_rom_raddr_final;
            logic [$clog2(2*(4**j))-1:0] twiddle_stage_rom_raddr_intt;
            assign twiddle_stage_rom_raddr_intt = (2*4**j-1) - twiddle_stage_rom_raddr[i][j];
