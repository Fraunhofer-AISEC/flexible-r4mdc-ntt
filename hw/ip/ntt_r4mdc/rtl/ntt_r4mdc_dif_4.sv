...
(
    ...
    logic [DATA_WIDTH-1:0] data_mdc2post0 [NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] data_mdc2post1 [NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] data_mdc2post2 [NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] data_mdc2post3 [NOF_BUTTERFLY_UNITS-1:0];
    logic [NOF_BUTTERFLY_UNITS-1:0] data_valid_mdc2post;
    ...
    for (genvar i=0; i<(NOF_BUTTERFLY_UNITS); ++i) begin : g_mdc_par
        ...
        ntt_r4mdc_dif_mdc #(
            ...
            .data0_o(data_mdc2post0[i]),
            .data1_o(data_mdc2post1[i]),
            .data2_o(data_mdc2post2[i]),
            .data3_o(data_mdc2post3[i]),
            .data_valid_o(data_valid_mdc2post[i])
        );       

    end : g_mdc_par

    logic [DATA_WIDTH-1:0] post_mdc_stage_din_0 [$clog2(NOF_BUTTERFLY_UNITS)/2-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] post_mdc_stage_din_1 [$clog2(NOF_BUTTERFLY_UNITS)/2-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] post_mdc_stage_din_2 [$clog2(NOF_BUTTERFLY_UNITS)/2-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] post_mdc_stage_din_3 [$clog2(NOF_BUTTERFLY_UNITS)/2-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [$clog2(NOF_BUTTERFLY_UNITS)/2-1:0] post_mdc_stage_data_valid_in;

    // Assign inputs to internal signals
    assign post_mdc_stage_din_0[$clog2(NOF_BUTTERFLY_UNITS)/2-1] = data_mdc2post0;
    assign post_mdc_stage_din_1[$clog2(NOF_BUTTERFLY_UNITS)/2-1] = data_mdc2post1;
    assign post_mdc_stage_din_2[$clog2(NOF_BUTTERFLY_UNITS)/2-1] = data_mdc2post2;
    assign post_mdc_stage_din_3[$clog2(NOF_BUTTERFLY_UNITS)/2-1] = data_mdc2post3;
    assign post_mdc_stage_data_valid_in[$clog2(NOF_BUTTERFLY_UNITS)/2-1] = data_valid_mdc2post;

    logic [DATA_WIDTH-1:0] post_mdc_stage_dout_0 [$clog2(NOF_BUTTERFLY_UNITS)/2-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] post_mdc_stage_dout_1 [$clog2(NOF_BUTTERFLY_UNITS)/2-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] post_mdc_stage_dout_2 [$clog2(NOF_BUTTERFLY_UNITS)/2-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] post_mdc_stage_dout_3 [$clog2(NOF_BUTTERFLY_UNITS)/2-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [$clog2(NOF_BUTTERFLY_UNITS)/2-1:0] post_mdc_stage_data_valid_out;    
    ...
    assign data0_o = post_mdc_stage_dout_0[0];
    assign data1_o = post_mdc_stage_dout_1[0];
    assign data2_o = post_mdc_stage_dout_2[0];
    assign data3_o = post_mdc_stage_dout_3[0];

    assign data_valid_o = post_mdc_stage_data_valid_out[0];

    for (genvar i=0; i<$clog2(NOF_BUTTERFLY_UNITS)/2; ++i) begin : g_post_mdc
        ...
        prim_ntt_r4gsbfu_stage_unrolled #(
            ...
            .data0_i(post_mdc_stage_din_0[i]),
            .data1_i(post_mdc_stage_din_1[i]),
            .data2_i(post_mdc_stage_din_2[i]),
            .data3_i(post_mdc_stage_din_3[i]),
            .data_valid_i(post_mdc_stage_data_valid_in[i]),

            .data0_o(post_mdc_stage_dout_0[i]),
            .data1_o(post_mdc_stage_dout_1[i]),
            .data2_o(post_mdc_stage_dout_2[i]),
            .data3_o(post_mdc_stage_dout_3[i]),
            .data_valid_o(post_mdc_stage_data_valid_out[i]),
    ...
    for (genvar i=1; i<$clog2(NOF_BUTTERFLY_UNITS)/2; ++i) begin
        assign post_mdc_stage_din_0[i-1] = post_mdc_stage_dout_0[i];
        assign post_mdc_stage_din_1[i-1] = post_mdc_stage_dout_1[i];
        assign post_mdc_stage_din_2[i-1] = post_mdc_stage_dout_2[i];
        assign post_mdc_stage_din_3[i-1] = post_mdc_stage_dout_3[i];
        assign post_mdc_stage_data_valid_in[i-1] = post_mdc_stage_data_valid_out[i];
    end
