...
static void print_usage() {
  std::cout
    << "Usage:\n"
    << "  test_top_ntt.exe -s <core-id> <target> <platform> [--device <index|bdf>] [--tv-root <path>]\n\n"
    << "Example:\n"
    << "  ./test_top_ntt.exe -s \"aisec:fpga:top_ntt_u55c_33550337_1024_16:0.1\" 33550337_1024_dit u55c\n\n"
    << "Arguments:\n"
              ...
              const fs::path& tv_root)
{
  ...
  for (size_t k = 0; k < batch_size; ++k) { 
    ...
    make_testvector_paths(tv_root, q_ps, n_ps, mode, std::to_string(k), coeff_in, coeff_exp);
    std::cout << "Reading testvectors:\n  " << coeff_in  << "\n  " << coeff_exp << std::endl;
    ...
    if (mode == "dit") { 
      for (size_t j = 0; j < (data_size/4)/16; ++j) {
        for (size_t i = 0; i < 16; ++i) {
          boCoeff0_map[k*data_size/4+i+j*16] = buf[j+i*data_size/(4*16)];
          boCoeff1_map[k*data_size/4+i+j*16] = buf[data_size/4+j+i*data_size/(4*16)];
          boCoeff2_map[k*data_size/4+i+j*16] = buf[data_size/2+j+i*data_size/(4*16)];
          boCoeff3_map[k*data_size/4+i+j*16] = buf[data_size/2+data_size/4+j+i*data_size/(4*16)];
          ref0[k*data_size/4+i+j*16] = buf_exp[4*j+i*data_size/16];
          ref1[k*data_size/4+i+j*16] = buf_exp[4*j+1+i*data_size/16];
          ref2[k*data_size/4+i+j*16] = buf_exp[4*j+2+i*data_size/16];
          ref3[k*data_size/4+i+j*16] = buf_exp[4*j+3+i*data_size/16];
        }
      }
    } else {
      for (size_t j = 0; j < (data_size/4)/16; ++j) {
        for (size_t i = 0; i < 16; ++i) {
          boCoeff0_map[k*data_size/4+i+j*16] = buf[4*j+i*data_size/16];
          boCoeff1_map[k*data_size/4+i+j*16] = buf[4*j+1+i*data_size/16];
          boCoeff2_map[k*data_size/4+i+j*16] = buf[4*j+2+i*data_size/16];
          boCoeff3_map[k*data_size/4+i+j*16] = buf[4*j+3+i*data_size/16];
          ref0[k*data_size/4+i+j*16] = buf_exp[j+i*data_size/(4*16)];
          ref1[k*data_size/4+i+j*16] = buf_exp[data_size/4+j+i*data_size/(4*16)];
          ref2[k*data_size/4+i+j*16] = buf_exp[data_size/2+j+i*data_size/(4*16)];
          ref3[k*data_size/4+i+j*16] = buf_exp[data_size/2+data_size/4+j+i*data_size/(4*16)];
        }
      }
    }
  }
...
int main(int argc, char** argv) {
  ...
  try {
    bool ok = run_test<int>(core_id, parameterset, platform, device_opt, tv_root);
    return ok ? 0 : 2;
  } catch (const std::exception& e) {
