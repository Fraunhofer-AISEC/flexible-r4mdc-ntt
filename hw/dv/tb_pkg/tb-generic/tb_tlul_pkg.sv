...
package tb_tlul_pkg;
    ...
    task automatic write_imem_from_file_tlul(
        input integer log_filehandle, 
        ref logic clk,
        ref integer clk_cycles,
        ...
        while (! $feof(imem_filehandle)) begin 
            ...
            write_tlul(.log_filehandle(log_filehandle), .clk(clk), .clk_cycles(clk_cycles), .data(instr), .address(otbn_reg_pkg::OTBN_IMEM_OFFSET+start_address+4*rel_addr), .tl_o(tl_o), .tl_i(tl_i) );
            rel_addr = rel_addr + 1;
        end 

        $fclose(imem_filehandle);

    endtask : write_imem_from_file_tlul


    task automatic write_dmem_from_file_tlul(
        input integer log_filehandle, 
        ref logic clk,
        ref integer clk_cycles,
        ...
        while (! $feof(dmem_filehandle)) begin 
            ...
            write_tlul(.log_filehandle(log_filehandle), .clk(clk), .clk_cycles(clk_cycles), .data(data), .address(otbn_reg_pkg::OTBN_DMEM_OFFSET+start_address+4*rel_addr), .tl_o(tl_o), .tl_i(tl_i) );
            rel_addr = rel_addr + 1;
        end 

        $fclose(dmem_filehandle);

    endtask : write_dmem_from_file_tlul
