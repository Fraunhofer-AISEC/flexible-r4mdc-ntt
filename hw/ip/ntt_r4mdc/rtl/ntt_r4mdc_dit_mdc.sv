...
#(
    ...
    parameter int unsigned DELAY_MULT = 9,
...
(
    ...
    assign mdc_stage_din_0[0] = data0_i;
    assign mdc_stage_din_1[0] = data1_i;
    assign mdc_stage_din_2[0] = data2_i;
    assign mdc_stage_din_3[0] = data3_i;
    assign mdc_stage_valin[0] = data_valid_i;

    // Assign outputs 
    assign data0_o = mdc_stage_dout_0[$clog2(N)/2-1];
    assign data1_o = mdc_stage_dout_1[$clog2(N)/2-1];
    assign data2_o = mdc_stage_dout_2[$clog2(N)/2-1];
    assign data3_o = mdc_stage_dout_3[$clog2(N)/2-1];
    assign data_valid_o = mdc_stage_valout [$clog2(N)/2-1];
    ...
    generate
        for (i=0; i < $clog2(N)/2; i++) begin
            ...
            if (i<$clog2(N)/2-1) begin
                ...
                logic [DATA_WIDTH-1:0] ctbf_in0_d;
                logic [DATA_WIDTH-1:0] ctbf_in0_q;

                logic [DATA_WIDTH-1:0] ctbf_in1_d;
                logic [DATA_WIDTH-1:0] ctbf_in1_q;

                logic [DATA_WIDTH-1:0] ctbf_in2_d;
                logic [DATA_WIDTH-1:0] ctbf_in2_q;

                logic [DATA_WIDTH-1:0] ctbf_in3_d;
                logic [DATA_WIDTH-1:0] ctbf_in3_q;

                assign ctbf_in0_d = mdc_stage_din_0[i];
                assign ctbf_in1_d = mdc_stage_din_1[i];
                assign ctbf_in2_d = mdc_stage_din_2[i];
                assign ctbf_in3_d = mdc_stage_din_3[i];

                always_ff @(posedge clk_i) begin
                    ctbf_in0_q = ctbf_in0_d;
                    ctbf_in1_q = ctbf_in1_d;
                    ctbf_in2_q = ctbf_in2_d;
                    ctbf_in3_q = ctbf_in3_d;
                end
                ...
                logic cnt_en;
                assign cnt_en = mdc_stage_valin[i];
