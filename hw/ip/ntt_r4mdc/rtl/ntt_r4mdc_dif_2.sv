...
(
    ...
    localparam PREMDC_STAGES  = (NOF_BUTTERFLY_UNITS==2) ? 1 :
    ...
    logic [DATA_WIDTH-1:0] pre_mdc_stage_din_0 [PREMDC_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] pre_mdc_stage_din_1 [PREMDC_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] pre_mdc_stage_din_2 [PREMDC_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] pre_mdc_stage_din_3 [PREMDC_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [PREMDC_STAGES-1:0] pre_mdc_stage_data_valid_in;


    logic [DATA_WIDTH-1:0] pre_mdc_stage_dout_0 [PREMDC_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] pre_mdc_stage_dout_1 [PREMDC_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] pre_mdc_stage_dout_2 [PREMDC_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] pre_mdc_stage_dout_3 [PREMDC_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [PREMDC_STAGES-1:0] pre_mdc_stage_data_valid_out;    

    logic [DATA_WIDTH-1:0] twiddle1_rom2mdc[PREMDC_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] twiddle2_rom2mdc[PREMDC_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];
    logic [DATA_WIDTH-1:0] twiddle3_rom2mdc[PREMDC_STAGES-1:0][NOF_BUTTERFLY_UNITS-1:0];

    logic [NOF_BUTTERFLY_UNITS-1:0] data_valid_mdc2pre;

    assign pre_mdc_stage_data_valid_in[PREMDC_STAGES-1] = data_valid_mdc2pre[0];

    assign data2mdc0 = data0_i;
    assign data2mdc1 = data1_i; 
    assign data2mdc2 = data2_i; 
    assign data2mdc3 = data3_i; 

    assign data_valid2mdc = data_valid_i; //pre_mdc_stage_data_valid_out[PREMDC_STAGES-1];

    assign data0_o = pre_mdc_stage_dout_0[0];
    assign data1_o = pre_mdc_stage_dout_1[0];
    assign data2_o = pre_mdc_stage_dout_2[0];
    assign data3_o = pre_mdc_stage_dout_3[0];
    assign data_valid_o = pre_mdc_stage_data_valid_out[0];
    for (genvar i=0; i<PREMDC_STAGES; ++i) begin : g_pre_mdc
        ...
        if ((NOF_BUTTERFLY_UNITS >> (i+1)*2) > 0) begin
            ...
            prim_ntt_r4gsbfu_stage_unrolled #(
            ...
            assign pre_mdc_stage_data_valid_out[i] = pre_mdc_stage_2bfus_data_valid_out[0];

            for (genvar j=0; j<(NOF_BUTTERFLY_UNITS/2); ++j) begin
                ntt_r4mdc_dif_stage_perm #(
                    .DATA_WIDTH(DATA_WIDTH),
                    ...
                    .data0_i(pre_mdc_stage_din_0[i][2*j+1:2*j]),
                    .data1_i(pre_mdc_stage_din_1[i][2*j+1:2*j]),
                    .data2_i(pre_mdc_stage_din_2[i][2*j+1:2*j]),
                    .data3_i(pre_mdc_stage_din_3[i][2*j+1:2*j]),
                    .data_valid_i(pre_mdc_stage_data_valid_in[i]),

                    .data0_o(pre_mdc_stage_dout_0[i][2*j+1:2*j]),
                    .data1_o(pre_mdc_stage_dout_1[i][2*j+1:2*j]),
                    .data2_o(pre_mdc_stage_dout_2[i][2*j+1:2*j]),
                    .data3_o(pre_mdc_stage_dout_3[i][2*j+1:2*j]),
    ...
    for (genvar i=1; i<PREMDC_STAGES; ++i) begin
        assign pre_mdc_stage_din_0[i-1] = pre_mdc_stage_dout_0[i];
        assign pre_mdc_stage_din_1[i-1] = pre_mdc_stage_dout_1[i];
        assign pre_mdc_stage_din_2[i-1] = pre_mdc_stage_dout_2[i];
        assign pre_mdc_stage_din_3[i-1] = pre_mdc_stage_dout_3[i];
        assign pre_mdc_stage_data_valid_in[i-1] = pre_mdc_stage_data_valid_out[i];
    end
    ...
    for (genvar i=0; i<(NOF_BUTTERFLY_UNITS); ++i) begin : g_mdc_par
        ...
        ntt_r4mdc_dif_mdc #(
            ...
            .data0_o(pre_mdc_stage_din_0[PREMDC_STAGES-1][i]),
            .data1_o(pre_mdc_stage_din_1[PREMDC_STAGES-1][i]),
            .data2_o(pre_mdc_stage_din_2[PREMDC_STAGES-1][i]),
            .data3_o(pre_mdc_stage_din_3[PREMDC_STAGES-1][i]),
            .data_valid_o(data_valid_mdc2pre[i])
        );       
